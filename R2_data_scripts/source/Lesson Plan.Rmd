...
2.  Explore data (base)
    ...
    4.  **Activity: Find the issues** [with a script? optional]
3.  correct some issues by re-loading with options
...
4.  Scripts
    ...
    2.  *Comments [the sooner this gets introduced, the sooner I can use them in examples!]*
    3.  Housekeeping: rm(list=ls())
    4.  open a script for the workshop code
    5.  Run code from the script
    6.  **Activity: readxl** in a script [need info on options, check results]
5.  Explore with dplyr: select() & filter()
    ...
    6.  **Activity: filter()**
    7.  piping
    8.  **Activity (short): convert expressions to pipes**
6.  Find issues systematically
...
8.  Conditional execution: if () else () -\> if_else(), case_when()
    1.  **Activity?**
9.  stringr + mutate: clean character columns
    1.  clean columns in example data

    2.  package:: object notation to use tidyr::fill()

    3.  **Activity: modify some strings (script)**
10. Grouped data
    1.  group_by()
    2.  summarise()

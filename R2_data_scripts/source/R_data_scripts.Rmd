...
str( dplyr::mutate(DF) )  # to drop extra attributes
```
...
DF_latin1 <- read_csv("data/data_example_latin1.csv", show_col_types = FALSE)
utf8_ex <- DF_latin1 |>
  dplyr::select(Type) |>
  dplyr::filter( ! Type %in% c("Quebec", "Mississippi") ) |> 
  unlist() |> unique()
...
## Read at a csv file with a mismatched encoding
...
- We will use "make.names" to make *syntactically valid* names

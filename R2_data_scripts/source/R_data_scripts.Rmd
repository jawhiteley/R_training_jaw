...
- Activities/Exercises, however, should generally **stand alone**
  - Activities/Exercises are an opportunity to try to apply knowledge, experiment, sometimes fail, and discuss 
...
+ Does the `filter()`{.r} verb use 'select' or 'mutate' _semantics_?
...
- Load rectangular (tabular) data into R
...
::: {.block data-latex="{}\itshape\small"}
The `readr` package provides functions for reading text data
in a variety of common formats (csv, tsv, fixed-width, etc.).
See the "Extras" document and package documentation
(`help(package = "readr")`{.r}) for details.
:::
...
read_csv(CSV_path)
```
```{r load with read_csv (1st try), echo=FALSE, results='hide', message=FALSE, warning=TRUE}
library(readr)
...
+ Use comments to
  ...
  <!--   based on conditions later. -- (see Extras) -->
...
- Add comments to explain your commands,
...
- Many $\R$ users find it useful to include this line near the top of their scripts,
  to ensure a "clean workspace" for the rest of the script.

- This can avoid problems with objects from different scripts 
  that have the same name, 
  or other unexpected behaviours from extra objects in your workspace.

::: notes
This was mentioned in the last workshop: this is a reminder
:::
...
- Use the script file in the "`activities`" folder as a starting point:
  - "`activity03_readxl.R`"
...
::: notes
Skip this activity if running late.
:::
...
* MULTIPLE TABLES
  + combining datasets
    ...
    - cbind / bind_cols
    - join() [mention merge()?]
    - Exercise: summarize() a dataset and join back with original for calculation?
...
# Explore Data With `dplyr`: `select()`{.r} & `filter()`{.r} Parts of a Data Frame
...
## **Activity 4**: explore data with `filter()`{.r}

- Explore the `iris` dataset (`?iris`{.r})
  with `filter()`{.r}, `select()`{.r}, and helper functions.

- Use the script file in the "`activities`" folder:
  - "`activity04_filter.R`"

- The script contains some questions for you to answer,
...
## Find non-numeric values in a character column {.shrink}
...
::: notes
https://stackoverflow.com/questions/21196106/finding-non-numeric-data-in-a-data-frame-or-vector
:::
...
<!--  the idea for this activity is incorporated into other activities
## **Activity ?**: detect values in data

- Use `filter()`{.r}, `select()`{.r}, and helper functions,
  to locate specific values _systematically_
...
## **Activity 6**: `mutate()`{.r} some columns

- Practice using `mutate()`{.r} and helper functions
  to add & modify columns in the `iris` dataset (`?iris`{.r}).
...
## **Activity 7**: `mutate()`{.r} with conditions

- Practice using `mutate()`{.r} and conditional expressions
  to add & modify columns in the `iris` dataset (`?iris`{.r}).

- Use the script file in the "`activities`" folder:
  - "`activity07_mutate_conditions.R`"
...
- The `675` column had some values where a comma (,)
  was used instead of a period (.) for the decimal.

- If we loaded the data without using "," as a grouping mark,
  the `675` column would be character.
  
```{r Find non-numeric values in `675` column, echo = FALSE}
...
```{r clean `675` column (character), results='hide'}
CSV_comma %>% select(1:3, "675") %>% 
  mutate(
    # Replace "," with "."
    `675` = str_replace(`675`, ",", "."),
...
## Clean some columns in the example data
...
<!-- could also use str_replace() -->

## Convert character column to numeric (drop characters)

- The `X500` column had a value with extra text:
  "`r DF_nonum[["X500"]]`"
...
- Remove non-numeric characters from `X500` column, and convert to numeric:
  ...
  - We can use `word()` from the `stringr` package
    to extract the first 'word' (separated by spaces ' ') 
    in each row.
...
- Other `stringr` functions would also work, such as `str_split_i()`{.r}


## Modify extreme values

- `X675` column had some large values where a comma (,)
  was used instead of a period (.) for the decimal,
  resulting in values 10\times{} what they should be.
  
```{r Extreme values, echo=FALSE, purl=FALSE}
DF_extreme_nums %>% knitr::kable()
```
...
## Fill in missing values (with `tidyr`)

- The '`Treatment`' column has some empty values

```{r unique values of the Treatment column}
...
## **Activity 8**: work with strings using the `stringr` package

- Practice using functions from the `stringr` package
  to explore and modify the `starwars` dataset (`?starwars`{.r}).

- Use the script file in the "`activities`" folder:
  - "`activity08_stringr.R`"
...
```{r grouping columns are excluded from operations, results='hide', message=FALSE}
DF %>% group_by(Type, PlantNum) %>% 
...
- Without groups specified, `summarize()`{.r}
  treats the data frame as a single group

```{r summarize ungrouped}
DF_clean4_trt %>% summarize(n(), mean(X95))
```

- But `summarize()`{.r} is most useful when applied to grouped data

```{r summarize grouped, results='hide'}
DF_clean4_trt %>% group_by(Type, PlantNum) %>% 
  summarize(n = n(), sum(X95))
```

- `summarize()`{.r} automatically drops the last level from the groups.
- `summarize()`{.r} & `summarise()`{.r} are synonyms (same function).


## **Activity 9**: `summarize()`{.r} data frames

- Practice using `summarize()`{.r} and helper functions
  to analyze and explore the `CO2` dataset (`?CO2`{.r}).

- Use the script file in the "`activities`" folder:
  - "`activity09_summarize.R`"
...
- Luckily in this case, the missing values
  in the duplicate rows look like they are _not_ missing
  in the other row
...
## Combine duplicate rows in example data? (to\ be\ continued...)

- We could collapse the duplicate rows using `summarize()`{.r}...
...
- It will be much easier to remove duplicates after the data is _tidy_
...
```{r Duplicate values of `uptake`, message=FALSE}
DF_tidy %>% 
...
- We can confirm _systematically_ that all duplicate values
  are equivalent (i.e., `min()`{.r} == `max()`{.r})
  - `near()`{.r} checks for equality, 
    with a tolerance for floating-point values.
    
```{r Confirm that all duplicate values are "near" the max & min}
DF_tidy %>% 
  group_by(Plant, conc) %>% 
  summarize(
    n = n(), 
...
- Now we only have to group by other columns,
  and `summarize()`{.r} the `uptake` column
  
  - and sort (with `arrange()`{.r}) to recover original order.
...
- Convert _all_ character columns to *factors*

```{r convert *all* character columns to factors, results='hide'}
DF_clean %>% 
  mutate( across(where(is.character), factor) )
```

<!-- to preserve the order of the factor levels:
**CAUTION: HTML comments will NOT prevent a R code chunk from running**
...
## **Activity 10**: reshape data with the `tidyr` package

- Practice using `pivot_wider()`{.r} and `pivot_wider()`{.r}
  to reshape the original `CO2` dataset (`?CO2`{.r}).

- Use the script file in the "`activities`" folder:
  - "`activity10_pivot.R`"

- The script contains some challenges for you to try,
  described in code *comments*.



## Tidy data makes plotting and analysis easier {.shrink}

\footnotesize
```{r plots are easier with tidy data, message=FALSE}
library(ggplot2)
DF_final %>% 
...
- It depends!
  ...
  - In other cases, you may want to pivot _wider_
    to separate different groups of values, 
    so that they can be cleaned differently.
...
## **Activity 11**: write data to files with the `readr` package

- Practice using `write_csv()`{.r} and other `tidyverse` functions
  to write portions of the `CO2` dataset (`?CO2`{.r}) to external files.

- Use the script file in the "`activities`" folder:
  - "`activity11_write.R`"

- The script contains some tasks for you to try to achieve,
  as *comments* in the code.
...
```{r bind_rows() stack data vertically, results='hide'}
...
- Combines columns together,
  with rows in the order they appear
  
  - **No attempt to match by ID**
  
  - Number of rows must be a compatible
  
```{r bind_cols() stack data horizontally, message=FALSE, results='hide'}
...
- Two-table verbs allow you to 'join' two data frames, 
  matching values in common.
  
  - similar to SQL joins
  
- See [`vignette("two-table")`](https://dplyr.tidyverse.org/articles/two-table.html)
  for more information.
  
::: notes
We don't have time to explore these today.
:::


## **Activity 12**: combine data frames with 2-table `dplyr` verbs

- Read the files you created in the last activity,
  then use `bind_cols()`{.r} or `bind_rows()`{.r}
  to combine the data frames.

- Use the script file in the "`activities`" folder:
  - "`activity12_2tables.R`"

- The script contains some specific tasks for you to try,
  described in code *comments*.
...
- Code that is written in a different style than you're used to,
  or with inconsistent formatting,
  can be difficult to read and follow.

<!--
- Code that is formatted consistently can be easier to read and understand.
-->
...
- Load *all* the data files in this directory:  
  ...
  - They are from the "World Health Organization Global Tuberculosis Report",
    with counts of new cases in each year for a subset of countries,
    by group (method of diagnosis, gender, and age)
    <!-- `?tidyr::who` -->
...
- _optional_: Save the data in the `data` directory
...
1. Which country had the highest all-time rate of new cases?
  1. In which year was it?
2. Which country has had the _lowest_ average rate since 2006 
   (including that year)?
3. Which country had the highest & lowest *growth rate* in *cases* since 2006 
   (including that year)?
4. What changed in 2006?
...
+ If multiple packages have functions with the same name,
  how can you [specify which one to use](#package-object-notation)?
+ What are ["UTF-8"](#encoding-non-english-characters) 
  and ["latin1"](#read_csv-read-a-file-with-a-different-encoding)?
+ What's the difference between a [dplyr 'verb'](#dplyr-verbs) and a ['helper function'](#dplyr-a-grammar-of-data-manipulation)?
+ Does the [`filter()`{.r} verb](#dplyr-verbs) use 'select' or 'mutate' [_semantics_](#dplyr-semantics)?
+ What are the [3 rules of 'tidy data'](#tidy-data)?
+ TRUE or FALSE: If $\R$ reads a file [without errors](#load-a-csv-file-into-r), there are no problems
+ TRUE or FALSE: $\R$ has rules and conventions for [naming functions](#the-tidyverse)
+ TRUE or FALSE: if you use one 
  [package from the `tidyverse`](#core-tidyverse-packages), 
  you have to use all of them.

::: notes
`## Answers (for discussion)`

+ Specify which function with `package::object` notation

+ "UTF-8" and "latin1" are character encodings
  - "UTF-8" is increasingly standard (even in $\R$),
    but some Windows programs will write files in "latin1"

+ dplyr 'verbs' are the main functions that act on a data frame
  - helper functions work _inside_ verbs to add functionality

+ `filter()`{.r} uses _mutate_ semantics
  - it expects a _vector of values_ to filter on

+ the 3 rules of 'tidy data' are:
  * Each variable must have its own column.
  * Each observation must have its own row.
  * Each value must have its own cell.

+ FALSE: Just because there were no errors reading the data,
  does **not** mean there are no problems.

+ FALSE: `tidyverse` has naming conventions; base $\R$ does not,
  though R does have a few _rules_ about naming functions

+ FALSE: you do **not** have to use all tidyverse packages
  --- you can load only the ones you want,
  or load the 'core' packages with `library(tidyverse)`
:::
...
```{r END of workshop script, include = FALSE}
################################################################
...
CSV files & encoding:
https://open.canada.ca/en/working-data#toc1a

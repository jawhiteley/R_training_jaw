...
\defbeamertemplate{background canvas}{refcardbg}{%
  \tikz\node[opacity=0.1]{%
    \includegraphics[width=\paperwidth,height=\paperheight,page=1]{images/Baggott-refcard-v2.pdf}
  };
}
...
\makeatletter
  \define@key{beamerframe}{refcardbg}[true]{%
    \setbeamertemplate{background canvas}[refcardbg]%
  }
\makeatother
...
\item
  \textbf{Press
  \textcolor{darkred}{\texttt{return}}/\textcolor{darkred}{\texttt{enter}}
...
\end{Shaded}
  ...
  \item
    ...
    \emph{quote} \textcolor{darkred}{\texttt{"}}, \emph{parenthesis}
    \textcolor{darkred}{\texttt{(}}, \emph{bracket}
    \textcolor{darkred}{\texttt{[}}, or \emph{brace}
    \textcolor{darkred}{\texttt{\{}}
  \end{itemize}
\item
  \StringTok{You can \textit{cancel} the current command at any time by pressing \texttt{escape}}
  (\textcolor{darkred}{\texttt{esc}})
\end{itemize}
\end{frame}
...
\item
  ...
  use the up \& down \textcolor{darkred}{arrow keys} (↑↓) to re-produce
  previous commands.
\item
  This lets you ``scroll through your \emph{command history}''.
\item
  Press \textcolor{darkred}{up} (\textcolor{darkred}{$\uparrow$}) once,
  and you get the last command you entered\\
...
\begin{column}{0.25\textwidth}
\centering
\textcolor{darkred}{\Large\tikzmarknode{index}{\textbf{\textit{index}}:}\\ the item\\ number}
\end{column}
...
\begin{column}{0.25\textwidth}
\centering
\textcolor{darkred}{\Large\textbf{\tikzmarknode{value}{\textit{value}}}}
\end{column}
\end{columns}
...
``equal'' --- \textcolor{darkred}{Note the \textbf{two} equals signs.}\\
...
\item
  \textbf{You still need the parentheses\textcolor{darkred}{()} !}
\item
  The same word without \textcolor{darkred}{\texttt{()}} refers to an
  \emph{object} (\emph{variable}) \emph{name}:\\
  adding the \textcolor{darkred}{\texttt{()}} specifies a \emph{function
  call}
\item
...
\item
  ...
  \tightlist
  \item
    \textcolor{darkred}{\textit{It is up to you to review the warnings and decide if they are important}}.
  \item
...
\begin{frame}[refcardbg]{HELP: Reference cards / cheat sheets}
...
\tightlist
\item
  ``\VariableTok{\texttt{Treatment}}'' column (name)
\end{itemize}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
\begin{Shaded}
...
\tightlist
\item
  elements 1-6 of \VariableTok{\texttt{Treatment}} column
\end{itemize}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
\begin{Shaded}
...
\item
  rows where\\
  \VariableTok{\texttt{Treatment}} == ``chilled''
\end{itemize}\strut
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
\begin{Shaded}
...
\item
  ...
  \end{itemize}
\item
  You can do this within \(\texttt{R}\), using simple functions
\item
  This keeps \(\texttt{R}\) lean, fast, and avoids ``feature bloat'':
...
\begin{itemize}
\item
  This often happens when you run \(\texttt{R}\) code written on a
  different computer, or written by someone else (on a different
  computer).
\item
...
\textcolor{darkred}{\textbf{This is all you need
...
\end{Shaded}
\item
  Take turns using the variable `\VariableTok{\texttt{x}}' as an
  \emph{argument} in a \emph{function} or \emph{expression}
\item
...
\tightlist
\item
  Change the object \emph{type} of \VariableTok{\texttt{x}} into a :
  ...
  \end{itemize}
\item
  Use \VariableTok{\texttt{x}} in a graph or
  \VERB|\FunctionTok{plot}\NormalTok{()}|
...
\item
  ...
  for?
\item
  \protect\hyperlink{why-is-it-named-textttr}{Why is it named
  '\(\texttt{R}\)'}?
\item
  How can you use \(\texttt{R}\)

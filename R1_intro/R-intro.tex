...
\begin{tikzpicture}[box/.style={ultra thick, rounded corners, draw=darkred, fill=white, fill opacity=0.5, text opacity=1},
                    ...
                    note/.style={font=\tiny, rounded corners, gray, fill=lightgray, fill opacity=.2, text opacity=1, inner sep=4pt},
...
% \textheight is larger than what actually fits in the frame body, even with the footer removed by the [plain] frame option. :(
  \begin{scope}[x={(image.south east)},y={(image.north west)}]
    ...
    \node (args_default) [note, align=left, anchor=165, text width=2.5cm] at (0.58,0.74) 
      ...
      \draw let \p1 = (args_default.165) in [arr] (args_default.165) -- (.25,\y1) -> (.25,.695);
    % Add labels for default values?
    \node (details) [lab, align=right, anchor=east, text width=4cm] at (0,0.05) 
...
\begin{tikzpicture}[box/.style={ultra thick, rounded corners, draw=darkred, fill=white, fill opacity=0.5, text opacity=1},
  ...
  \begin{scope}[x={(image.south east)},y={(image.north west)}]
    \node[lab, align=right, anchor=north east] at (0,0.9) 
      {Details};
  \end{scope}
\end{tikzpicture}
\end{frame}

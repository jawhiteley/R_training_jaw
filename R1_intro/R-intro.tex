...
\protect\hypertarget{why-is-it-named-includegraphicsheight1emcprogra1rr-411.3dochtmllogo.jpg}{}
\begin{itemize}
\tightlist
\item
...
\item
  ...
  \uline{\textbf{R}}obert Gentleman at the University of Aukland in the
  early 1990s.
\end{itemize}
\end{frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{demo}\NormalTok{(graphics)  }\CommentTok{\# }
\FunctionTok{demo}\NormalTok{(image)     }\CommentTok{\# }
\FunctionTok{demo}\NormalTok{(lm.glm)    }\CommentTok{\# }
\FunctionTok{demo}\NormalTok{()          }\CommentTok{\# }

\FunctionTok{help.start}\NormalTok{()    }\CommentTok{\#}
\end{Highlighting}
\end{Shaded}

A great place to start, especially if you are comfortable reading
...
R parser), which results in an unevaluated \emph{expression}, which can
in turn be evaluated (by `the evaluator', such as \texttt{eval()}) to
return the \emph{value}.

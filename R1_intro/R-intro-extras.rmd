...
but anything else triggers an \highlight{error}
```{r, eval=FALSE}
0myvar <- F
_myvar <- F
my var <- F
...
- Names with any character are actually possible, but must be quoted with backticks (```)
  - **This is not recommended practice**, but occasionally useful when you need to refer to an element of an object, such as lists items or data frame columns, that have non-standard names.

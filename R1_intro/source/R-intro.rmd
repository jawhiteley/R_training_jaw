...
## https://stackoverflow.com/questions/76964346/can-you-specify-different-folders-for-tex-and-pdf-output-files-in-r-markdown
knit: 
  (function(input_file, ...) {
...
```{r debugging_page, eval=FALSE, include=FALSE, purl=FALSE}
## Debugging
\small
...
```{r debugging, echo=FALSE, eval=FALSE, purl=FALSE}
...
Pronoun sharing ("go-rounds") can be a minefield, but useful, and should always be optional:
- Free account required: https://medium.com/national-center-for-institutional-diversity/making-space-for-them-her-him-and-prefer-not-to-disclose-in-group-settings-why-1deb8c3d6b86
- https://pronouns.org/asking
- https://www.theequalityinstitute.com/equality-insights-blog/why-and-how-to-share-pronouns-at-work
- https://www.nytimes.com/2021/04/29/style/pronouns-gender-work-social-qs.html
...
## Interacting with $\Rlogo$ {#interacting-with-R-command-line}
...
+ Expected output (results) look like this:
  ...
  ```{r format_output, include=TRUE, eval=TRUE, echo=FALSE, purl=FALSE}
  cat("Output (results)")
  ```
...
::: notes
Note the name of the variable in quotes with `assign()`:
the name of the variable is declared as a character string,
making it easier to build variable names dynamically.
:::


## Retrieve values {.shrink}
...
retrieve the value of a variable, using the name as a character string
:::
...
Some words and letters already have special meaning in the R language (*keywords*)[^reserved-words] 
and should **never be used as variable names**.
...
[^reserved-words]: See section 10.3.3 of "[The R Language Definition](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Reserved-words)"
...
- The same word without \important{\texttt{()}} refers to an *object* (*variable*) *name*:  
...
<!-- not as familiar with this one:
- Quik-R: [www.statmethods.net](https://www.statmethods.net) 
  - by [datacamp](https://www.datacamp.com/)
-->
...
- Cheat sheets from RStudio/Posit: 
  ...
  - [Contributed Cheatsheets](https://rstudio.github.io/cheatsheets/contributed-cheatsheets.html), including [non-English translations](https://posit.co/resources/cheatsheets/?type=translations/)
...
:::
::: {.column width="40%"}
[![](images/Baggott-refcard-v2.pdf)](https://cran.r-project.org/doc/contrib/Baggott-refcard-v2.pdf)
[![](images/base-r.pdf)](https://github.com/rstudio/cheatsheets/blob/main/base-r.pdf)
:::
::::::
...
https://rstudio.github.io/cheatsheets/translations.html
Posit Cheat Sheets:
...
## <!-- $\R$ Documentation: read it --> {#R-documentation-read-it .shrink}
...
## <!-- $\R$ Documentation: read it (end) -->  {#R-documentation-read-it-end .shrink}
...
| ```                            |   "CRAN mirror"[^CRAN] (server) to use.     |
|                                |                                             |
|                                | \important{\textbf{%                        |
...
```{r install_car, echo=FALSE, results='hide', message=FALSE}
## execute the code
install.packages("car")
library(car)
```

[^CRAN]: [C]{.underline}omprehensive [R]{.underline} [A]{.underline}rchive [N]{.underline}etwork: repositories of R software, packages, and documentation.
...
```{r command-r-examples, results='hide'}
x <- x + 1
x <- x * (x+10)
x <- log(x)
x <- exp(x)
...
There is a script file containing all the code 
shown in the slides for this workshop:
'`R-intro.R`' in the ['`source'` folder](https://github.com/jawhiteley/R_training_jaw/tree/main/R1_intro/source)
of the workshop files.
...
Most IDEs have a shortcut to send portions of $\R$ code (a line or statement over multiple lines) to an $\R$ session:
...
- [**R\ Markdown**](https://rmarkdown.rstudio.com/) is another file type that can store reusable $\R$ code.

- R Markdown combines $\R$ code with text and other content,
  written in *Markdown*.

- Markdown is a simple plain text format
  that is both human-readable and machine-readable.
  
  - Markdown can be easily converted to
    formatted document types (especially ones based on a mark-up language),
    including HTML (web pages), pdf, or even Word and PowerPoint files.

- Combining $\R$ code and other content 
  supports reproducible research,
  and can allow for automated reports.
  
- ["R Markdown: The Definitive Guide"](https://bookdown.org/yihui/rmarkdown/) 
  contains much more information about R Markdown, itself written in R Markdown.


## R Markdown & RStudio

* R Markdown files have a '`.rmd`' file extenstion,
  but they are also a plain text format.
  - They can be edited in any text editor, and compiled using
    the `rmarkdown` package in any flavour of $\R$ or IDE.

* RStudio makes working with R Markdown files easy, with features like
a 'visual' editing mode, 
and built-in tools to compile (*render*) documents.

### Notes

The slides for this workshop were also made using R Markdown!  
You can find the `.rmd` file in the ['`source'` folder](https://github.com/jawhiteley/R_training_jaw/tree/main/R1_intro/source).


## R Notebooks

- RStudio lets you use any R Markdown file like a *notebooks*,
  where you can run code chunks and see the output,
  either in the document or in your console.
  - But the results will not be saved if you close the file.

- Results and output can be saved by producing an "html_notebook" output,
  which can be shared along with the rmd file
  for reproducible research and analysis.

- More information on R Notebooks is available in the [Notebook chapter](https://bookdown.org/yihui/rmarkdown/notebook.html) of the [R Markdown book](https://bookdown.org/yihui/rmarkdown/).


### Notes

1. You can open the "`.rmd`" file used to produce this document in RStudio,
   and use it as an 'R Notebook' to run each 'code chunk'
   and see the results, without having to copy & paste code into the console.
  
2. We'll explore script files and notebooks more in the next workshop.
...
+ What does '[CRAN](#installing-the-car-package)' stand for?
+ [Why is it named '$\R$'](#why-is-it-named-textttr)?
+ How can you use $\R$ [_interactively_](#interacting-with-R-command-line)?
+ How do you find out [what a function does](#textttr-documentation-find-it) & [how to use it](#R-documentation-read-it-end)?
+ How do you [store values](#symbolic-variables) to [re-use later](#retrieve-values)?
+ True or False: [Warnings](#warnings) can be ignored, but an [Error](#errors) means I made a mistake.
+ True or False: [Error messages](#errors) will tell me how to fix the problem.

::: notes
`## Answers (for discussion)`

+ What does 'CRAN' stand for?
  - [Comprehensive R Archive Network](#installing-the-car-package)
  - a repository of current and previous R software, packages, and documentation, with mirrored copies on servers around the world.
+ [Why is it named '$\R$'](#why-is-it-named-textttr)?
  - An open-source implementation of the `S` statistical language
  - Created by Ross Ihaka and Robert Gentleman (aka "R & R"), whose names both start with the letter "R"
+ How can you use $\R$ _interactively_?
  - By entering commands in the [command-line](interacting-with-R-command-line),
    and reading messages and output from the *console*.
+ How do you find out what a function does & how to use it?
  - [Documentation](#textttr-documentation): e.g., the various [`help()` functions](#textttr-documentation-find-it)
  - [`example()` function](#R-documentation-read-it-end)
  - [Web sites](#help-web-sites); search the internet
  - [Books](#help-books)
  - [etc.](#help-documentation)
+ How do you store values to re-use later?
  - [Symbolic variables](#symbolic-variables)
  - [*assign* the value to a *name*](#symbolic-variables)
  - Use the names in code later to [retrieve the stored value](#retrieve-values)
+ True or False: Warnings can be ignored, but an Error means I made a mistake.
  - FALSE (trick question): 
  - [You have to figure out if a Warning can be safely ignored](#warnings)
  - An Error isn't necessarily your fault, but [it does mean something happened that R could not handle](#errors).
+ True or False: Error messages will tell me how to fix the problem.
  - FALSE
  - [Error messages will tell you what went wrong](#errors), 
    and maybe even where the problem is;
  - but they will rarely tell you how to fix it: you have to figure that out for yourself.
:::




## References & More Information {.squeeze}
...
- https://education.rstudio.com/teach/tools/
  - https://education.rstudio.com/teach/tools/#packages
...
```{r NOTES, include=FALSE, eval=FALSE, purl=FALSE}
...
https://tex.stackexchange.com/questions/173201/beamer-template-with-different-style-options-for-frames

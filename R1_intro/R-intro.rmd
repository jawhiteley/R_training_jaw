...
header-includes: |
  ...
  \setlength{\parskip}{\baselineskip}   % incease space between regular paragraphs, without affecting lists
  \newcommand{\Rlogo}{\includegraphics[height=1em]{`r file.path(R.home("doc"), "html", "logo.jpg")`}}
  \newcommand{\R}{\texttt{R}}
  \newcommand{\ctop}{\vspace{\OuterFrameSep}}  % for aligning text with the top of a code block in a table
  ...
  \newcommand{\name}[1]{\textit{\VariableTok{\texttt{#1}}}}
---
...
\begin{tikzpicture}[remember picture, overlay, 
  ...
  \draw[arr] (index.east) -- ++(0.5cm,0cm) -- ++(0,0.2cm) -> ++(0.4cm,0cm);
  \draw[arr] (index.east) -- ++(0.5cm,0cm) -- ++(0,-1.1cm) -> ++(0.6cm,0cm);
  \draw[arr] (value.west) -> ++(-6cm,0.7cm);
  \draw[arr] (value.west) -- ++(-5.9cm,0cm) to ++(0cm,-0.3cm);
\end{tikzpicture}
...
\begin{tikzpicture}[remember picture, overlay, 
  ...
  \begin{scope}[shift=(code.south west),x=(code.south east),y=(code.north west)]
    \draw[arr] ($ (as.north) +(1ex,1.5ex) $) -- ($ (0,0) +(1ex,\OuterFrameSep+0.5ex) $);
  \end{scope}
\end{tikzpicture}
...
Computers can't represent _all_ values accurately, and there is often some rounding that occurs (even at 50+ decimal places).  As a result, 'floating point' values may not be _reliably equal_.
...
<!-- updates: clarify use of "=", default values? -->
...
- arguments can be *passed by order* or *passed by name*
  ...
  - *passed by order* means the arguments are specified in the correct order

  - *passed by name* means the arguments can be in any order, but must be declared by name.  
    e.g.: `argument = value`{.r}
...
- When R receives a command it does not understand, or cannot execute, it outputs an ***error*** to the *console*.
  - This is in the form of a message that begins with the word "\ErrorTok{\texttt{Error}}".
...
- Some commands still work, but did not run exactly as $\R$ (or the developers) think is "ideal", and may produce a ***warning*** instead.
  - This is in the form of a message that begins with the word "\WarningTok{\texttt{Warning}}".
...
+ ***Messages*** are for information, and a sign that things are working fine (at least, according to the programmers who created the function).

...
- There is nothing wrong with the file, 
  but this indicates a _mismatch_ between 
...
- "[`UTF-8`](https://en.wikipedia.org/wiki/UTF-8)" 
  ...
  - But *Windows* uses "`latin1`" by default,
    and so does $\R$ when running in Windows.
...
- Conversely, if reading a file that was 
...
but it adds extra contents to the file (a "Byte-Order Mark" or "BOM") 
that makes it difficult to read with `read.csv()`{.r}.
See the "Extras" file of this workshop for details on how to deal with that.  

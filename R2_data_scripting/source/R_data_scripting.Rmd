...
+--------------------+-----------------------------+--------------------+-----------------------+
| `read_csv2()`{.r}  | '`;`' as delimiter\         | `read_csv2()`{.r}  | '`,`' for decimals,\  |
...
- `read_csv()`{.r} printed a helpful message summarizing what it did,
  including _guessing the data type_ of each column.


## Tibbles: streamlined `data.frame`s
...
- Use the functions we [learned earlier](#head-peek-at-the-first-few-rows) 
  to inspect the results of the two functions.
...
- `read_csv()`{.r} produces a `tibble` object,
  not a simple `data.frame`

## Options to control reading files
...
+ exercise: clean some values in Example File loaded earlier?
  - I want to use that file for demos, to ensure everyone's objects are the same.
-->

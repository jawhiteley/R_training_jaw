...
+ defining your own functions
  ...
  - write a custom function for comparing a dataset to CO2, to check our work?
    - incorporate conditional execution, messages, warnings?
...
√ read_csv()
  ...
  √ options for controlling behaviour: clumn names, column classes, NAs
...
- `arrange()` changes the order of rows of a data frame
  based on the values of selected columns

```{r arrange() examples, results='hide'}
arrange(CO2, conc)
...
- The last example uses **helper functions**
  - `across()`{.r} lets you choose columns to sort by, based on criteria
  - `where()`{.r} is a _selection-helper_ function
    that applies the `is.character()`{.r} _function_ to each column, 
    returning a `TRUE`{.r} or `FALSE`{.r} value,
    to use as the criteria for `across()`{.r}.
    
::: notes
More on helper functions and semantics later
:::
-->
...
## Find problematic values systematically {.shrink}
...
* **"mutate semantics"** (`<data-masking>`): 
  in `mutate()`{.r}, a column name refers to a _vector of values_.
...
* Many helper functions only work in one context or the other,
...
- To clean character columns, we will use functions from the `stringr` library,
...
## Collapse groups: `summarise()`{.r}

## `summarise()`{.r} multiple columns with `across()`{.r}
...
* You can also define ad hoc functions using a special "lambda" notation
  - refer to the value of the column with '`.x`'

```{r summarise() multiple columns with across(), results='hide'}
DF %>% 

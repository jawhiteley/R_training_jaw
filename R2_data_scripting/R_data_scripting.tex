...
\item
  ...
  \tightlist
  \item
    or ``\emph{Session \textgreater{} Set Working Directory
    \textgreater{} To Files Pane Location}'' in the R~Studio menu.
  \end{itemize}
\end{itemize}
\end{column}

\begin{column}{0.38\textwidth}
\includegraphics[width=0.8\textwidth,height=\textheight]{images/rstudio-set-wd-files.png}
\end{column}
\end{columns}
...
\tightlist
\item
  R Studio also has a menu item to conveniently set the working
  directory to the same location as a file in the `Source' pane.\\
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{file.exists}\NormalTok{(}\StringTok{"data\_example.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\end{verbatim}
\item
  If the result of the statement above is not
  ``\VERB|\ConstantTok{TRUE}|'' in your session, try one of the
  \protect\hyperlink{set-the-working-directory}{other approaches} to
  change your working directory, and try again.
\end{itemize}
\end{frame}
...
\tightlist
\item
  Just because there were no \VERB|\NormalTok{Error}|s from \(\R\),
...
\begin{frame}[fragile,shrink]{Look at a column}
...
\begin{block}{Tip}
\protect\hypertarget{tip}{}
\emph{the \VERB|\FunctionTok{str}\NormalTok{()}| and
\VERB|\FunctionTok{names}\NormalTok{()}| functions can be used with
\textbf{any} object}
\end{block}
\end{frame}

\begin{frame}[fragile,shrink]{Simple plots}
...
\item
  ...
  \tightlist
  \item
    The \VERB|\FunctionTok{View}\NormalTok{()}| pane in R Studio
    (default upper-left; `\emph{Source}') also allows for sorting and
    filtering, but these do not change the viewed object, only the view.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{\texttt{dplyr} \emph{semantics}}
\protect\hypertarget{dplyr-semantics}{}
\texttt{dplyr} verbs and helper functions let you refer to column names
of the data frame directly in their arguments as regular variables ---
without having to quote them. But these names have different meanings
(semantics) in different verbs.

\begin{itemize}
\tightlist
\item
  \textbf{``select semantics''}: in
  \VERB|\FunctionTok{select}\NormalTok{()}| and similar functions, a
  column name refers to its \emph{position} in the data frame.

  \begin{itemize}
  \tightlist
  \item
    you can refer to a column as a quoted string in
    \VERB|\FunctionTok{select}\NormalTok{()}|, and it is interpreted as
    a reference to the column.
  \end{itemize}
\item
  \textbf{``mutate semantics''}: in
  \VERB|\FunctionTok{mutate}\NormalTok{()}|, a column name refers to a
  \emph{vector of values}.

  \begin{itemize}
  \tightlist
  \item
    you cannot supply a column name as a string in
    \VERB|\FunctionTok{mutate}\NormalTok{()}|, because it is treated as
    a vector of length 1, rather than a reference to a column of values.
  \end{itemize}
\end{itemize}

\note{\url{https://dplyr.tidyverse.org/articles/dplyr.html\#patterns-of-operations}
\VERB|\FunctionTok{vignette}\NormalTok{(}\StringTok{"dplyr"}\NormalTok{)}|}
\end{frame}
